...
function create_file_server() {
  ...
  docker run -d -p $FILE_SERVER_PORT:80 -v $REPO_OUTPUT/$VERSION_ID:/usr/share/nginx/html/$VERSION_ID -v $FILES_OUTPUT:/usr/share/nginx/html/files --restart=always --name $FILE_SERVER_NAME $FILE_SERVER_IMAGE
  if [ $? -ne 0 ]; then
