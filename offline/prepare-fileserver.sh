...
function create_file_server() {
  ...
  if [ -n "$docker_ps" ]; then
    ...
    docker run -d -p $FILE_SERVER_PORT:80 -v $REPO_OUTPUT:/usr/share/nginx/html/$NAME-$VERSION_ID -v $FILES_OUTPUT:/usr/share/nginx/html/files --restart=always --name $FILE_SERVER_NAME $FILE_SERVER_IMAGE
    if [ $? -ne 0 ]; then
