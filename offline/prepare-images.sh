...
function create_registry() {
  ...
  docker run -d -p $REGISTRY_PORT:5000 --restart=always --name $REGISTRY_NAME $REGISTRY_IMAGE
  if [ $? -ne 0 ]; then
...
function pushed_images() {
  ls -la $IMAGES_OUTPUT/*.tar | awk '{print $NF}' > $IMAGES_OUTPUT/image-tar.list
  images_tar=$(cat $IMAGES_OUTPUT/image-tar.list)
