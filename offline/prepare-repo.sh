...
[local-mirror]
name=Local Mirror
baseurl=http://$current_ip:$FILE_SERVER_PORT/repository/$NAME-$VERSION_ID/$ARCH
enabled=1
gpgcheck=0
EOF
    echo "Creating done."
...
deb [trusted=yes] http://$current_ip:$FILE_SERVER_PORT/repository/$NAME-$VERSION_ID/$ARCH/ /
EOF
    echo "Creating done."
  fi
}
